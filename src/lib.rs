...
impl From<protobuf::Vote> for raft_types::Vote {
    fn from(proto_vote: protobuf::Vote) -> Self {
        let leader_id: LeaderId = proto_vote.leader_id.unwrap().into();
        if proto_vote.committed {
            raft_types::Vote::new_committed(leader_id.term, leader_id.node_id)
        } else {
            raft_types::Vote::new(leader_id.term, leader_id.node_id)
        }
    }
}
...
impl From<protobuf::VoteRequest> for VoteRequest {
    fn from(proto_vote_req: protobuf::VoteRequest) -> Self {
        let vote: raft_types::Vote = proto_vote_req.vote.unwrap().into();
...
impl From<protobuf::VoteResponse> for VoteResponse {
    fn from(proto_vote_resp: protobuf::VoteResponse) -> Self {
        let vote: raft_types::Vote = proto_vote_resp.vote.unwrap().into();
...
impl From<raft_types::Vote> for protobuf::Vote {
